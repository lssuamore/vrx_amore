...
#include "std_msgs/Float32.h"							// for reading in buoy locations from visions
...
bool NED_waypoints_published = false;			// false means the NED poses have not yet been calculated and published
bool NED_waypoints_recieved = false;			// false means goal poses have not been acquired from "waypoints_NED" OR they haven't been converted
...
bool navigation_array_initialized = false;							// false means navigation_array is not initialized
bool propulsion_system_initialized = false;						// false means propulsion_system is not initialized
bool perception_array_initialized = false;							// false means the perception_array is not initialized
bool mission_control_initialized = false;							// false means the mission_control is not initialized
...
{
	if ((mission_control_initialized) && (MC_state == 0) && (MC_state != 5))								// if the system is initialized and mission_control is on standby and NOT IN CHANNEL NAVIGATION MODE
	{
...
int main(int argc, char **argv)
{
  ...
  // RECOMMENDATION: Put all buoy data into one message and setup just one function and subscriber 
  ros::Subscriber left_right_buoy_array_sub = nh8.subscribe("left_right_buoy_array", 1, left_right_buoy_loc_update);				// subscribes to left and right buoy locations in global frame
  
  // Publishers
  current_goal_pose_pub = nh12.advertise<amore::usv_pose_msg>("current_goal_pose", 1);						// current goal for low level controller (propulsion_system)
  ps_state_pub = nh13.advertise<amore::state_msg>("ps_state", 1);																		// current propulsion_system state
  na_state_pub = nh14.advertise<amore::state_msg>("na_state", 1);																		// current navigation_array state
  pa_state_pub = nh15.advertise<amore::state_msg>("pa_state", 1);																		// current perception_array state
  ...
  while(ros::ok())
  {
	  ...
	  state_update();		// For buoy_nav_planner only ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  else if (MC_state == 5)				// TASK 5: CHANNEL NAVIGATION
	  {
		  if (!calculations_done)
		  {
			  calculate_buoy_waypoints();
		  }
		  else if (loop_count > loop_goal_recieved)
		  {
			  if (PS_state == 1)
			  {
				  current_goal_pose_publish();
				  ...
				  if ((e_xy < e_xy_allowed) && (!E_reached))
				  {
					  point += 1;
					  ROS_INFO("Point %i reached. --MC", point);
					  if (point==goal_poses)
	  ...
	  } // if (MC_state == 5)				// TASK 5: CHANNEL NAVIGATION
